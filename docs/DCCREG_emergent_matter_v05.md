...
**Version:** 0.5 (Matter / fermion-gate block)
...
3. **Mass mechanism — RESOLVED in mechanism (run004, §7a); scale open.** Inter-sublattice (L–R) coupling **does** generate a Dirac mass (the net-$\mathrm{Lk}$-0 mixing of the two opposite-handed $L=8$ ribbons) — [IR], computed. **Remaining:** the mass *scale* ($\propto J$), which needs $J$ and $K_A$ in physical units (couple to run002); and whether any scale matches a real fermion (stays [RH], §8).
