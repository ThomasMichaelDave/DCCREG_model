...
**Version:** 0.2 (Matter / fermion-gate block)
...
**Status after the joining (v0.2).** Three pieces are now computed without the simulator: the **charges** (chiral octahedral group $O$: {90°,120°,180°}; §2a = IV-§3a), the **coupling** ($K_A\sim\beta\sim\rho\Gamma^2\ln(d/a)$, shared with light isotropy and gravity strength; §2a, IV-§3a), and the **spin-parity selection** (Fibonacci near-closure of the screw framing; §6a). **The simulator boundary** — which loops are *stable* (energies), the lowest-energy matter loop's exact spin and handedness, and the precise numeric $K_A$ (lattice Biot–Savart sum) — remains. The joining closed the *structural* gate-sharing and the *parametric* coefficient; the numeric spectrum is the simulator task. The $[1,1,1]$ screw makes the relevant problem quasi-2D (transverse-to-screw), as in IV-§5.7. **[OC for the computed pieces; the numeric spectrum is open]**
...
**Open forks.**
1. **Defect spectrum (= gravity $K_A$).** Charges (octahedral $O$, §2a), coupling ($K_A\sim\beta$, §2a), and spin-parity selection (Fibonacci, §6a) are **computed**. Remaining (simulator, quasi-2D): the stable-loop **energies**, the lowest-energy matter loop's exact spin/handedness, and the numeric $K_A$. Closes IV-§10.1 / IV-§3a.
