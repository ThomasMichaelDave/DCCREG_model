...
**Version:** 0.3 (Matter / fermion-gate block)
...
**Status after run002 (v0.3).** The simulator boundary the joining left open is now substantially crossed (validation ladder 5/5 PASS). **Computed:** the numeric coupling $K_A\approx0.316\,\rho\Gamma^2$ (IV-§3a); the **ground-state matter loop** — charge $120^\circ$ (§2b), $|\mathrm{Lk}|=1$ → **spin-½**, definite handedness, unique Fibonacci closure $L=8$ (§6a); and the categorical charge selection (90° excluded, 120° by $\Omega^2$ + screw alignment, §2b). **Still open:** the full multiplet / excited spectrum beyond the ground state, and the [RH] map to real particle quantum numbers (§8). So the gate is closed at the level of the *ground state's* spin/charge/handedness; the *spectrum above it* and the *particle identity* remain. **[OC for the computed pieces; spectrum-above and SM-map open]**
...
**Open forks.**
1. **Defect spectrum — ground state CLOSED (run002, §2b/§6a/§9).** Numeric $K_A\approx0.316\rho\Gamma^2$; ground state = spin-½, charge 120°, handedness, $L=8$; charge fixed categorically. Closes IV-§10.1 / IV-§3a at ground-state level. **Remaining:** the **excited/multiplet spectrum** above the ground state (does a generation structure or a doublet appear?), and convergence of the $E(r)$ coefficient beyond ~10% (finite-torus).
