...
**Version:** 0.6 (Gravity-gate block)
...
1. **Disclination coefficient — residual after closure (§5.5, §5.7).** The power-law is **closed at $1/k^2$** (Einstein form; hexatic disclination kernel = Laplacian Green's function). Remaining is *quantitative*: the coefficient $K_A$ (Frank constant; an $\ln(d/a)$-type ratio, analog of III's $\beta/\alpha$), the genuine **3D** coefficient (quasi-2D $\ln r$ → 3D $1/r$ stacking), and the static-vs-dynamic-shear refinement (§5.7). Identical to the fermion-gate disclination spectrum (item 8). Simulator-addressable.
