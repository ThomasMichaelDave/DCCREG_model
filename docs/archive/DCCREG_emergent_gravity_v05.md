...
**Version:** 0.5 (Gravity-gate block)
...
| **Is the dynamics Einstein?** | **Static/Newtonian sector firmed to $1/k^2$** (dominant Coulomb channel, §6.8); the defect (curvature) interaction **leans Einstein robustly** — the SOSF is **hexatic** (§5.5), so dislocation-screening gives $1/k^2$, converging with the Coulomb channel. Closure = the quasi-2D disclination coefficient (exact-Einstein vs Newtonian-form), still open | [IR; firmed-leaning, coefficient open] |
...
1. **Disclination-interaction coefficient — closure of the gate (§5.5, §5.7).** The power-law is firmed to $1/k^2$ (hexatic dislocation-screening + Coulomb channel). Remaining: the explicit **quasi-2D (transverse-to-screw) disclination computation** for the *coefficient* — whether it is *exactly* linearized Einstein or merely Newtonian-form, and the 3D status. Identical to the fermion-gate disclination spectrum (item 8). Simulator-addressable.
...
*Document status: [exploratory; internally consistent; tiers honest]. Block IV v0.5: the Fork-1 conditional is resolved structurally — the SOSF is **hexatic** (fixed orientation from the immutable skeleton, fluid position from I-§6), so proliferated dislocations screen the disclination interaction to **$1/k^2$ (Einstein form)**, converging with the §6.8 strong-Coulomb channel. Gravity is **Coulomb-gauge** (instantaneous strong Newtonian potential + weak finite-$c$ graviton); its weakness is the unscreened-residual result; the vacuum is a medium with a bounded screening length; $\alpha_1=0$ is forced. The gate **leans Einstein robustly**; closure needs the quasi-2D disclination *coefficient* (§5.7, §10.1), which is the same object as the fermion-gate disclination spectrum (§10.8). Still open: that coefficient, $\alpha_2$, the energy→softening sign, $\gamma$, dynamic-vs-static shear for light.*
