...
**Version:** 0.4 (Gravity-gate block)
...
### 5.5 The power-law leans Einstein because the substrate is a fluid (melted crystal)

The §5.4 binary is not actually a free choice — it is set by whether the gravity sector is **rigid or melted**, and the framework's locked assumptions resolve it.

- The relevant modulus is **shear $C_{66}$** (light/graviton sector). In a vortex lattice the long-range part of the inter-vortex interaction goes into the *compression* modulus (stiffened, here infinite — the incompressible/sound sector), while **shear is local and finite** — shear is volume-preserving and screened from the long-range piece (standard Abrikosov-lattice elasticity). A local first-gradient shear modulus gives the ordinary biharmonic defect law: disclinations interact via the confining $\sim r^2\ln r$ ($\sim 1/k^4$). **The bare *rigid* skeleton gives softer-than-Einstein.** **[OC]**
- But a finite local shear gives $1/k^4$ *only while dislocations are bound*. When they **proliferate/condense** — which is what melting a crystal into a fluid *is* (KTHNY; Kleinert's floppy world crystal) — the confinement is screened and the disclination interaction crosses to $\sim 1/k^2$: **linearized Einstein.** The DCCREG substrate is *locked* as an ideal incompressible Euler **fluid**, "plasma-like" (I-§0) — i.e. the *melted/screened* state. So the constitutive assumptions put the gravity sector on the **Einstein ($1/k^2$) side.** **[OC for the Kleinert/melting mechanism; IR for the identification with the locked fluid assumption]**

**Reconciling the two readings with §5.2.** The fixed-skeleton picture (I-§6, rigid, no transport) is the *compatible* sector — it carries the wave (light). The fluidity is what lets the *incompatible* (defect/gravity) sector proliferate. So the lattice is **rigid for light (compatible) and melted for gravity (incompatible)** — one medium, two responses; the apparent contradiction is the feature. **Net: the power-law leans Einstein, conditional on the substrate's fluidity (defect condensation) operating at the scale that sets the emergent metric** — the genuine remaining computation, identical to the fermion-gate defect-spectrum question (§10.8). **[IR]**
...
| **Is the dynamics Einstein?** | **Static/Newtonian sector firmed to $1/k^2$** (via the dominant Coulomb channel, §6.8); full nonlinear Einstein dynamics still hinges on §5.5 defect computation ($1/k^2$ vs $1/k^4$), which **leans Einstein** (fluid = melted crystal) | [IR; partly firmed, partly open] |
...
**Bottom line.** The framework has a single coherent picture: **one chiral vortex lattice whose compatible (defect-free) sector is light and whose incompatible (defect) sector is gravity-and-matter**, with gravity itself **Coulomb-gauge** — an instantaneous strong longitudinal Newtonian potential plus a weak finite-$c$ transverse graviton, exactly mirroring the electrodynamics. As of v0.4: the **static/Newtonian sector is firmed to $1/k^2$** (carried by the dominant Coulomb channel), **gravity's weakness is explained** (the unscreened residual of the screened-strong Coulomb sector — the standard large-scale-structure reason, reproduced), the vacuum is recognised as a **medium with a bounded screening length** (Einstein = the featureless-medium limit), $c_{\rm GW}=c_\gamma$ passes GW170817 structurally, and **$\alpha_1=0$ is forced** by the substrate's two defining features. What remains genuinely open: the full nonlinear *Einstein dynamics* (the §5.5 defect power-law, which leans Einstein via melting but needs the explicit computation), $\alpha_2$ at its singular limit, the energy→softening sign, and PPN $\gamma$. GR is recovered as the $\alpha_1,\alpha_2\to0$ *and* featureless-medium limit — a doubly-exceptional corner; the framework's divergences (preferred-frame parameters, screening length) are its falsifiable content and are tightly bounded. Honest net: the construction is markedly more unified and the *Newtonian* sector is now on firm Einstein-compatible footing with the weakness derived, but the *full Einstein dynamics* is firmed-leaning, not yet established.
...
*Document status: [exploratory; internally consistent; tiers honest]. Block IV v0.4: gravity is **Coulomb-gauge** — an instantaneous strong longitudinal Newtonian potential ($1/k^2$, the static attraction) plus the weak finite-$c$ transverse graviton; the **static/Newtonian sector is firmed to Einstein/Newtonian form** via the dominant Coulomb channel, and **gravity's weakness is explained** as the unscreened residual of the screened-strong Coulomb sector. The vacuum is a **medium** ("atmosphere") with a bounded screening length; Einstein is the featureless-medium limit. Still open: the full nonlinear Einstein dynamics (the §5.5 defect power-law, leaning Einstein via melting), $\alpha_2$, the energy→softening sign, $\gamma$. The fermion gate is the same defect object (§10.1/§10.8).*
