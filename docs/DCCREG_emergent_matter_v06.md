...
**Version:** 0.6 (Matter / fermion-gate block)
...
| 0.5 | 2026-05-30 | DCCREG Modeling Programme | **Dirac pairing — run004; the missing partner is inter-sublattice; the fixed-chirality axiom AFFIRMED (major).** Run `run004_dirac_pairing` (validation ladder + **two new gating rungs PASS**: $J\to0$ decoupling reproduces two independent run003 singlets; A↔B mirror invariance holds) tested whether run003's "no doublet" means the axiom is too strong, or whether the partner lives on the other sublattice. **Result: B-YES (the pre-committed branch).** New §7a: the two opposite-handed $L=8$ ribbons on the dual sublattices (I-§13) carry $\mathrm{Lk}=\pm1$ and are **exactly degenerate** ($E_A=E_B$ by mirror symmetry) [OC]; the **mutual-helicity coupling** ($|\mathrm{Lk}_{AB}|=1$, reinforcing per I-§13) binds them into a bonding/antibonding pair whose **net-$\mathrm{Lk}$-0 mixing is a Dirac mass term** [OC arithmetic / IR identification]. So the partner run003 found "missing" on one sublattice is present on the *other*, and the inter-sublattice coupling **is** the §8 Dirac-mass mechanism — promoting §8's "Mass is [RH]" to **[IR], computed in mechanism** (magnitude still ∝ $J$, unfixed without physical-unit $K_A$). **The fixed-global-chirality axiom (I-§4, locked) SURVIVES intact** — global handedness stays fixed (DC); the *local* dual structure already supplies both chiralities, and their coupling is the mass. **No Block I revision.** This is the first time a locked axiom was stress-tested with a pre-committed falsification branch (B-NO would have opened a Block I chirality revision) and held. §7a added; §8 mass bullet upgraded; §10 fork 3 closed; cross-ref V-§8 ↔ I-§13 added; footer updated. SM electroweak-isospin map stays [RH], fenced. The v0.1–v0.4 core is unchanged. |
| 0.6 | 2026-05-31 | DCCREG Modeling Programme | **Physical-scale reduction — run005 (accounting refinement; verdict B-REDUCED).** The reduction layer (ladder incl. dimensional-closure + known-limit-recovery rungs, all PASS) shows the matter masses do **not** reduce to pure numbers but to $\{\rho,\Gamma,\ell_P\}\times$ dimensionless $g(J,\ln(d/a))$: $m_D\sim\rho\ell_P^3\,(J/\ln(d/a))$. The cross-gate ratio $(m_D/m_{\rm Planck})^2\to J^2\ln(d/a)^{-3/2}$ (base scales cancel — **dimensionally**, the generic property of a dimensionless ratio on a 3-constant basis, not a structural result). First-principles $J=1$ ($|\mathrm{Lk}_{AB}|=1$, run004) [IR] fixes the $J$ residual, leaving an uncomputed $O(1)$ prefactor and the inexact $\ln(d/a)\approx4$ — so **no parameter-free mass ratio**. Introduces the emergent action $\hbar_{\rm eff}\sim\rho\Gamma\ell_P^3$ (the defect's angular momentum $=\hbar/2$), **[IR]** — which **presupposes the §10 fork 2 soliton quantisation (not carried out)**, so $\hbar_{\rm eff}$ is not a derived Planck constant and $m_{\rm Planck}$ is an internally-defined scale; the cross-gate ratio is therefore an **internal scaling relation, not yet a measurable prediction**. §9 status note; §10 forks 1/2/3 updated; bidirectional cross-ref to **IV-§3a/§10**. The SM-mass map stays **[RH], fenced (§8)**. No change to the v0.1–v0.5 core (spin-½ ground state, 120° charge, $K_A$, Dirac mechanism). |
...
## 7a. Dirac pairing — the partner is inter-sublattice; the axiom survives (run004, v0.5)

§7 leaves the matter natively **Weyl** (massless, single handedness). run003 then found the $L=8$ ground state is an **isolated singlet** with **no doublet**, because the *fixed global screw sense* (locked, I-§4) realises only the screw-aligned handedness — putting that locked axiom on the table. run004 tested the resolution, with **both outcomes pre-committed** (the brief's B-YES / B-NO branches), so a doublet could not be tuned into existence.

**The setup (the framework's own dual structure).** Block I-§13 already contains **two counter-rotating sublattices of opposite handedness**. The $L=8$ spin-½ ground ribbon exists on *both*: $\mathrm{Lk}=+1$ on sublattice A (screw $+1/3\varphi^2$), $\mathrm{Lk}=-1$ on B (mirror screw). run003 had modelled one sublattice; run004 couples the two.

**The result — B-YES.** [OC for the arithmetic; IR for the mass identification]
- The A and B ribbons are **exactly degenerate**, $E_A=E_B$, by the substrate's mirror-pseudoscalar symmetry (I-§1) — validated by the A↔B mirror-invariance gating rung, not assumed.
- Their **mutual helicity** is $|\mathrm{Lk}_{AB}|=1$ and **reinforcing** (opposite handedness ⇒ counter-rotating ⇒ constructive at the shared node, I-§13). This binds them: diagonalising $H=\bigl[\begin{smallmatrix}E_A&t\\t&E_B\end{smallmatrix}\bigr]$ ($t=J\,M_0$, $M_0=-1$) at the natural scale $J=1$ gives a **bonding** state $11.104$ (bound, below $E_0=12.104$) and **antibonding** $13.104$, **gap $\Delta=2.000$** — below the first-excited gap ($5.00$), so a genuine near-degenerate low-lying pair, not a split. The bonding state is bound for all $J>0$; the antibonding member merges with the excited band only at strong coupling $J\gtrsim2.5$.
- The bound state **mixes the two chiralities** ($\mathrm{Lk}_A=+1$ with $\mathrm{Lk}_B=-1$, net $0$): a term coupling left- and right-handed Weyl components is, structurally, a **Dirac mass**. The Weyl pair of §7 acquires a mass through the inter-sublattice coupling.

**What this means for the axiom.** The "missing partner" of run003 was on the *other sublattice* all along. So:
- the **fixed-global-chirality axiom (I-§4) survives intact** — global handedness (DC) stays fixed; the *local* dual structure (I-§13) supplies both chiralities;
- their coupling **is** the Dirac-mass mechanism that §8 had flagged [RH] — now **[IR], computed in mechanism** (the *magnitude* is $\propto J$, the inter-sublattice coupling, still unfixed in physical units without the run002 $K_A$);
- **no Block I revision is needed.** Had run004 returned B-NO (partner forbidden even with both sublattices), the brief committed to opening a Block I chirality revision with its upstream cost spelled out. It did not; the locked axiom was stress-tested and held. **[OC for the negative-branch logic; the axiom affirmation is the [IR] reading of a passed test]**

> **The Weyl→Dirac picture.** §7: each sublattice carries a massless Weyl defect of definite handedness. §7a: the two opposite-handed Weyl defects, coupled across the dual sublattices, form a **massive Dirac** object. Chirality is still primordial and global-fixed; mass is the inter-chirality (inter-sublattice) coupling. This is the standard Weyl→Dirac structure, realised by the substrate's dual geometry. The map to electroweak isospin doublets remains **[RH], fenced (§8)**.
...
- **Mass mechanism — now [IR] (run004, §7a); magnitude still open.** Weyl defects are massless; a Dirac mass needs left–right coupling. run004 **computed the mechanism**: the inter-sublattice coupling of the two opposite-handed $L=8$ ribbons (net-$\mathrm{Lk}$-0 mixing) **is** that Dirac mass — promoted from the sketched [RH] candidate to an [IR] result. *What remains [RH]/open:* the **magnitude** ($\propto J$, unfixed without physical-unit $K_A$), and whether this mass corresponds to any *particular* real fermion mass — not claimed.
...
**Status after run005 (v0.6) — physical-scale reduction (shared with IV-§3a).** The "absolute masses in physical units" item is addressed as *dimensional accounting*, not as SI numbers (by design). Result: the masses carry free inputs irreducibly — $m_D\sim\rho\ell_P^3\,(J/\ln(d/a))$, and the cross-gate $(m_D/m_{\rm Planck})^2\to J^2\ln(d/a)^{-3/2}$ with the base scales cancelling **dimensionally**. The framework currently makes input-independent *intra-sector* arithmetic predictions (the $3\varphi^2$ screw period, the $180/120$ charge ratio $\Rightarrow2.25$, the $L\approx8n$ winding ladder) and is **one $O(1)$ geometric prefactor + an exact $\ln(d/a)$ short of a parameter-free *cross-gate* gravity↔mass number**. Verdict **B-REDUCED**. **[OC dimensional structure; IR for the $\hbar_{\rm eff}/c$ identifications and "this ratio is a prediction"; RH for the SM map, §8]**
...
**Open forks.**
1. **Defect spectrum — ground state CLOSED, excited spectrum CHARACTERISED (run002+run003, §2b/§6a/§6b/§9).** Numeric $K_A\approx0.316\rho\Gamma^2$; ground state = isolated spin-½ singlet, charge 120°, handedness, $L=8$ (categorical charge). Excited band = winding harmonics $L\approx8n$, no doublet, no generation multiplet (§6b). **Remaining:** absolute defect **energies/masses in physical units** (couple §6b to the run002 $K_A$ machinery — not yet done); convergence of the $E(r)$ coefficient beyond ~10% (finite-torus, IV). *Run005 (accounting):* the energies/masses reduce to $\{\rho,\Gamma,\ell_P\}\times g(J,\ln(d/a))$ (no hidden input; closure PASS) — not SI numbers; the located gap to a parameter-free cross-gate ratio is an $O(1)$ prefactor + an exact $\ln(d/a)$ (IV-§3a/§10).
2. **Soliton quantisation.** Carry out the canonical/path-integral quantisation that promotes the classical framed defects to quantum fermions (the measure / Wess–Zumino-type term). *Note (run005):* the dimensional reduction introduced an emergent action $\hbar_{\rm eff}\sim\rho\Gamma\ell_P^3$ identified with the defect's spin-½ ([IR]). That identification **presupposes** this quantisation step; it is not a substitute for it. Until the measure is constructed, "$\hbar_{\rm eff}$ is the physical $\hbar$" and "the defect spin is exactly $\hbar/2$" remain [IR] resting on this open fork.
3. **Mass mechanism — RESOLVED in mechanism (run004, §7a); scale open.** Inter-sublattice (L–R) coupling **does** generate a Dirac mass (the net-$\mathrm{Lk}$-0 mixing of the two opposite-handed $L=8$ ribbons) — [IR], computed. **Remaining:** the mass *scale* ($\propto J$), which needs $J$ and $K_A$ in physical units (couple to run002); and whether any scale matches a real fermion (stays [RH], §8). *Run005 (accounting):* $m_D\sim\rho\ell_P^3\,(J/\ln(d/a))$ and $(m_D/m_{\rm Planck})^2\to J^2\ln(d/a)^{-3/2}$. With $J=1$ from run004 ($|\mathrm{Lk}_{AB}|=1$) the $J$ residual is fixed [IR], but an $O(1)$ prefactor and the inexact $\ln(d/a)$ keep the scale from being a pure number; still no match to a measured fermion claimed ([RH], §8).
