...
**Status (run005, verdict B-REDUCED).** Both gates' quantities now reduce to the free-input set $\{\rho,\Gamma,\ell_P\}\times g(J,\ln d/a)$ ($+\kappa$ EM, $+n_d$ screening; dimensional closure verified — no hidden input). The cross-gate $(m_D/m_{\rm Planck})^2\to J^2\ln(d/a)^{-3/2}$ — the base scales cancel *dimensionally* (the generic property of a dimensionless ratio on a 3-constant basis, **not** a structural result), and with the emergent $\hbar_{\rm eff}\sim\rho\Gamma\ell_P^3$ [IR] (resting on the open soliton-quantisation step, V-§10 fork 2) $m_{\rm Planck}$ is internally defined, so this is an internal scaling relation, **not yet a measurable prediction**. Today's input-independent predictions are *intra-sector* only ($3\varphi^2$ screw period, $180/120$ charge ratio $\Rightarrow2.25$, $L\approx8n$ winding ladder). **The next sharply-located target is the single $O(1)$ geometric prefactor + an exact $\ln(d/a)$** that would turn the cross-gate ratio into a parameter-free number. No SI absolutes are sought (by design). Block IV → **v0.9**, Block V → **v0.6** (accounting refinement). (run `run005_physical_scales`, `sim/checks/check_06_physical_scales.py`, ladder incl. dimensional-closure + known-limit-recovery rungs all PASS.)
