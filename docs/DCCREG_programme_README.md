...
| `DCCREG_emergent_matter_v04.md` | **Block V** — matter / fermion gate: screw-framing ⇒ ribbons; **spin-½** (belt trick / Finkelstein–Rubinstein); **automatic spin-statistics**; **spin = self-helicity, statistics = mutual helicity**; chirality ⇒ **Weyl**. Defect spectrum = the Block IV gravity coefficient | V-§ |
...
**Update (run002, ladder 5/5):** the defect spectrum's *ground state* is computed — $K_A\approx0.316\rho\Gamma^2$/$G_{\rm eff}$ (IV v0.8) and a spin-½, charge-120°, handed ground-state loop ($L=8$) (V v0.3). Remaining: the **excited/multiplet spectrum** above the ground state, α₂, energy→softening sign, γ, soliton quantisation, and the [RH] SM-particle map. Recommendation (updated, post-run003): the excited spectrum is characterised — no multiplet (V-§6b). The most useful next targets are **absolute defect energies/masses in physical units** (couple V-§6b's relative spectrum to run002's $K_A$ machinery), then the carried analytic forks **α₂** (IV) and the **energy→softening sign** (IV, decides whether gravity attracts).
